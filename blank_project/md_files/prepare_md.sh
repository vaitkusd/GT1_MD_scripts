...
# Main script that calls all other scripts and gromacs commands.
...
# generate lig topology
acpype -i $lig.pdb -b ${LIG} -o gmx
# fetch generated files
mv ${LIG}.acpype/${LIG}_GMX.gro ./${lig}.gro
mv ${LIG}.acpype/${LIG}_GMX.itp ./${lig}x.itp
# clean ligand topology (if 2 ligands are present)
# NB: copy removed atomtypes to upg.itp
...
######## STEERED MD PREPARATION ########
# not relevant when doing unconstrained MD
...
# extract catalytic histidine atom numbers (from precompiled file)
